...
class Node {
    ...
    // Düğümü canvas üzerine çiz - Modern tasarım
    draw(ctx) {
        ctx.save();

        // Gölge efekti (her zaman)
        ctx.shadowBlur = this.isHighlighted ? 25 : 12;
        ctx.shadowColor = this.isHighlighted ? this.color : 'rgba(0, 0, 0, 0.3)';
        ctx.shadowOffsetX = 0;
        ctx.shadowOffsetY = this.isHighlighted ? 0 : 4;

        // Dış daire - gradient dolgu
        const gradient = ctx.createRadialGradient(
            this.x - this.radius * 0.3, this.y - this.radius * 0.3, 0,
            this.x, this.y, this.radius
        );

        // Renk bazlı gradient oluştur
        ...
        // İç parlak halka (highlight efekti)
        ...
        // Label (düğüm adı) - gölgeli ve kontrastlı
        ctx.shadowBlur = 0;
    ...
    // Rengi açma yardımcı fonksiyonu
...
class Edge {
    ...
    draw(ctx) {
        ...
        ctx.lineWidth = this.isHighlighted ? this.lineWidth : 2.5;
        ctx.lineCap = 'round';
        ctx.stroke();

        // İkinci katman - parıltı efekti (highlight için)
        if (this.isHighlighted) {
            ctx.beginPath();
            ctx.moveTo(startX, startY);
            ctx.lineTo(endX, endY);
        ...
        // Ağırlık kutusu boyutları
        const padding = 6;
        const text = this.weight.toFixed(1);
        ...
        this.roundRect(ctx, midX - boxWidth / 2, midY - boxHeight / 2, boxWidth, boxHeight, 6);
        ctx.fill();

        // Kutu kenarlığı
        ctx.shadowBlur = 0;
        ctx.shadowOffsetY = 0;
        ctx.strokeStyle = this.isHighlighted ? this.color : 'rgba(100, 116, 139, 0.3)';
        ctx.lineWidth = 1;
        ctx.stroke();

        // Ağırlık değeri
        ctx.fillStyle = this.isHighlighted ? this.darkenColor(this.color, 30) : '#475569';
        ctx.font = 'bold 10px "Inter", Arial, sans-serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
    ...
    // Rengi açma yardımcı fonksiyonu
    lightenColor(color, percent) {
        if (!color || color.charAt(0) !== '#') return '#aabbcc';
        const num = parseInt(color.replace('#', ''), 16);
        const amt = Math.round(2.55 * percent);
        const R = Math.min(255, (num >> 16) + amt);
        const G = Math.min(255, ((num >> 8) & 0x00FF) + amt);
    ...
    // Rengi koyulaştırma yardımcı fonksiyonu
    darkenColor(color, percent) {
        if (!color || color.charAt(0) !== '#') return '#556677';
        const num = parseInt(color.replace('#', ''), 16);
        const amt = Math.round(2.55 * percent);
        const R = Math.max(0, (num >> 16) - amt);
        const G = Math.max(0, ((num >> 8) & 0x00FF) - amt);
...
class Graph {
    ...
    // Graf verisini düğüm özellikleri formatında CSV'ye dönüştür
    // Format: DugumId Ozellik_I (Aktiflik) Ozellik_II (Etkileşim) Ozellik_III (Bağl. Sayısı) Komsular
    toNodePropertiesCSV() {
        let csv = 'DugumId,Ozellik_I (Aktiflik),Ozellik_II (Etkileşim),Ozellik_III (Bağl. Sayısı),Komsular\n';

        this.nodes.forEach(node => {
            // Bu düğümün komşularını bul
            const neighbors = this.getNeighbors(node);
            const neighborIds = neighbors.map(n => n.node.id).join(',');

            // Özellik I: Aktiflik (0-1 arası normalize edilmiş değer)
            const aktiflikNormalized = (node.aktiflik / 100).toFixed(1);

            // Özellik II: Etkileşim (tam sayı)
            const etkilesim = Math.round(node.etkilesim);

            // Özellik III: Bağlantı Sayısı (gerçek komşu sayısı)
            const baglantiSayisi = neighbors.length;

            csv += `${node.id},${aktiflikNormalized},${etkilesim},${baglantiSayisi},"${neighborIds}"\n`;
        });

        return csv;
    }

    // Düğüm özellikleri CSV formatından graf yükle
    // Format: DugumId,Ozellik_I (Aktiflik),Ozellik_II (Etkilesim),Ozellik_III (Bagl. Sayisi),Komsular
    fromNodePropertiesCSV(csvText) {
        this.clear();
        ...
        // Önce tüm satırları parse et
        for (let i = 1; i < lines.length; i++) {
            // CSV satırını parse et (tırnak içindeki virgülleri koru)
            const cells = this.parseCSVLine(lines[i]);
            if (cells.length < 5) continue;

            const dugumId = parseInt(cells[0]);
            const aktiflik = parseFloat(cells[1]);
            const etkilesim = parseFloat(cells[2]);
            const baglantiSayisi = parseInt(cells[3]);
            const komsularStr = cells[4].replace(/"/g, '').trim();
            const komsular = komsularStr ? komsularStr.split(',').map(k => parseInt(k.trim())).filter(k => !isNaN(k)) : [];
        ...
        // Düğümleri dairesel yerleşimde oluştur
        const n = nodeDataList.length;
        const centerX = canvas.width / 2;
        const centerY = canvas.height / 2;
        const radius = Math.min(canvas.width, canvas.height) / 3;

        nodeDataList.forEach((nodeData, i) => {
            const angle = (2 * Math.PI * i) / n - Math.PI / 2;
            const x = centerX + radius * Math.cos(angle);
            const y = centerY + radius * Math.sin(angle);

            const node = new Node(nodeData.id, x, y);
            // Aktiflik 0-1 aralığındaysa 1-100'e çevir
            node.aktiflik = nodeData.aktiflik <= 1 ? Math.round(nodeData.aktiflik * 100) : nodeData.aktiflik;
            node.etkilesim = nodeData.etkilesim;
            node.baglantiSayisi = 0; // Kenarlar eklendiğinde güncellenecek
        ...
        // Bağlantıları oluştur (çift bağlantı önleme için kontrol)
        const addedEdges = new Set();
        nodeDataList.forEach(nodeData => {
            const sourceNode = nodeMap.get(nodeData.id);
            if (!sourceNode) return;

            nodeData.komsular.forEach(neighborId => {
                const targetNode = nodeMap.get(neighborId);
                if (!targetNode) return;

                // Çift bağlantı önleme
                const edgeKey = [Math.min(nodeData.id, neighborId), Math.max(nodeData.id, neighborId)].join('-');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // CSV Dışa Aktar (Düğüm Özellikleri)
    document.getElementById('exportCsvBtn').addEventListener('click', async () => {
        ...
        const csvContent = graph.toNodePropertiesCSV();
        const fileName = `sosyal_ag_${new Date().toISOString().slice(0, 10)}.csv`;
            ...
            if ('showSaveFilePicker' in window) {
                ...
                canvasManager.showResult(`
                    ...
                    <div style="text-align: center; padding: 10px;">
                        <span style="font-size: 28px;">📊</span>
                        <p style="color: #475569; margin-top: 8px;">
                            Düğüm özellikleri CSV formatında kaydedildi.
                        </p>
                        <p style="color: #64748b; font-size: 12px; margin-top: 4px;">
        ...
        reader.onload = (event) => {
            ...
            try {
                if (fileName.endsWith('.json')) {
                    ...
                    // CSV içe aktarım - otomatik format algılama
                    const firstLine = content.trim().split('\n')[0].toLowerCase();

                    // DugumId başlığı varsa düğüm özellikleri formatı
                    if (firstLine.includes('dugumid') || firstLine.includes('düğümid')) {
                        graph.fromNodePropertiesCSV(content);
                        canvasManager.updateStats();

                        canvasManager.showResult(`
                            <div class="success-message">
                                <span>✓</span> <strong>Düğüm özellikleri CSV içe aktarıldı!</strong>
                            </div>
                            <div style="text-align: center; padding: 10px;">
                                <span style="font-size: 28px;">📋</span>
                                <p style="color: #475569; margin-top: 8px;">
                                    Düğüm özellikleri tablosundan graf oluşturuldu.
                                </p>
                                <p style="color: #64748b; font-size: 12px; margin-top: 4px;">
                                    ${graph.nodes.length} düğüm, ${graph.edges.length} bağlantı
                                </p>
